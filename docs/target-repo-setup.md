...
For GitHub, use the official `github-mcp-server` as a lazy project capability plus the existing GitHub CLI/API release path. The Docker runtime installs the official binary and the example config starts it with `stdio`, dynamic tool discovery, and read-only mode. Dynamic discovery gives agents access to the full documented GitHub MCP toolset surface on demand without loading the entire catalog into every run. Use read-only scopes by default and enable write-capable GitHub tools only for delivery/release stages that need them. GitHub CLI should authenticate through `GH_TOKEN`/`GITHUB_TOKEN` or a deliberate mount of the user's `gh` config directory, not hardcoded credentials. The runtime mirrors `GH_TOKEN` or `GITHUB_TOKEN` into `GITHUB_PERSONAL_ACCESS_TOKEN` for the official MCP server when that MCP-specific variable is not set.

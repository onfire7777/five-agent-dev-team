...
| `github-read` | issues, PRs, checks, release state | GitHub MCP or existing GitHub API client | Use read-only scopes by default. |
...
- GitHub MCP for issue/PR/check/release context when a stage needs GitHub awareness
...
Do not let GitHub MCP memory or tool state bleed between repos. Use project-specific tokens or GitHub App installation scopes where possible.
